...
for btn_text in num_pad_list:
    def cmd(x=btn_text):  # 함수에 인수(숫자 키)를 전달
        click(x)
    Button(num_pad, width=11, height=3, text=btn_text, command=cmd).grid(row=r, column=c)
    c += 1
...
for btn_text in operator_list:
    def cmd(x=btn_text):  # 함수에 인수(연산자 키)를 전달
        click(x)
    Button(operator, text=btn_text, width=5, height=3, command=cmd).grid(row=r, column=c)
    c = c + 1
    if c > 1:
        c = 0
        r = r + 1
